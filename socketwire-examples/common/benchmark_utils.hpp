...
inline Options parseOptions(int argc,
                            const char** argv,
                            ...
                            std::uint16_t defaultGamePort = 10888)
{
  ...
  if (options.clients <= 0)
    options.clients = 1;
  if (options.metricsMode != "summary")
    options.metricsMode = "samples";

  return options;
}

inline float deterministicAxis(std::uint32_t seed, std::uint64_t frame, std::uint32_t axis)
{
  const auto phase = static_cast<std::uint32_t>((frame / 30 + seed * 17 + axis * 7) % 4);
  switch (phase)
  {
    case 0:
      return 1.f;
...
{
public:
  MetricsCollector(Options options, const char* example, const char* backend, const char* role)
  ...
  void maybeWriteSample()
  {
    ...
    if (!updateMeasurementState(now))
      return;

    if (options_.metricsMode == "summary")
      return;

    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - lastSample_).count() < 1000)
      return;
  ...
  void finish()
  {
    ...
    if (options_.enabled && measuring_)
      writeSample(Clock::now());
...
private:
  ...
  void writeSample(Clock::time_point now)
  {
    ...
    const auto elapsedMs = std::chrono::duration_cast<std::chrono::milliseconds>(now - measurementStart_).count();
    const auto sampleMs = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastSample_).count();
    const double sampleSeconds = sampleMs > 0 ? static_cast<double>(sampleMs) / 1000.0 : 1.0;
    const double currentCpu = nowCpuSeconds();
    const double cpuPercent = ((currentCpu - lastCpuSeconds_) / sampleSeconds) * 100.0;
    lastCpuSeconds_ = currentCpu;

    const double frameAvg = frameSamples_ > 0 ? frameMsSum_ / static_cast<double>(frameSamples_) : 0.0;
    const double updateAvg = updateSamples_ > 0 ? updateMsSum_ / static_cast<double>(updateSamples_) : 0.0;
    ...
    std::fprintf(file_,
                 ...
                 static_cast<unsigned long long>(payloadRxPackets_),
                 networkStats_.rttMs,
                 static_cast<unsigned long long>(networkStats_.lostPackets),
                 static_cast<unsigned long long>(networkStats_.inflightPackets),
                 static_cast<unsigned long long>(networkStats_.sendWindow),
                 frameAvg,
                 updateAvg,
                 cpuPercent,
                 static_cast<unsigned long long>(currentRssKb()));
    std::fflush(file_);
...
inline void recordPayloadTx(std::size_t bytes)
{
  if (activeCollector() != nullptr)
    activeCollector()->recordPayloadTx(bytes);
}

inline void recordPayloadRx(std::size_t bytes)
{
  if (activeCollector() != nullptr)
    activeCollector()->recordPayloadRx(bytes);
}
