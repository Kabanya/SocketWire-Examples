...
static std::uint32_t get_delta_data(
...
static void draw_ship(float ship_len, float ship_width, float x, float y,
...
static void draw_entity(const Entity& e) {
  ...
  draw_ship(
    ...
    ColorFromHSV(static_cast<float>(static_cast<int>(hsv.x + 120.f) % 360), 1.f,
                 1.f));
  draw_ship(SHIP_LEN, SHIP_WIDTH, e.x, e.y, fwd, left,
...
static void simulate_world(
  ...
  get_entity(myEntity, [&](Entity&) {
    ...
    totalOutData += static_cast<std::uint32_t>(
...
static void draw_world(const Camera2D& camera, const BandwidthAccumulator& bw) {
  ...
  DrawRectangleLines(-WORLD_SIZE, -WORLD_SIZE, 2.f * WORLD_SIZE,
                     2.f * WORLD_SIZE, WHITE);
  ...
  for (std::size_t x = 1; x < NUM_GRID; ++x)
    DrawLine(
      ...
      -WORLD_SIZE,
      -WORLD_SIZE + 2.f * WORLD_SIZE * (static_cast<float>(x) / NUM_GRID),
      WORLD_SIZE, GetColor(0xffffffff));
...
int main(int argc, const char** argv) {
  ...
  if (!benchOptions.enabled) InitWindow(width, height, "Ship Swarm");
