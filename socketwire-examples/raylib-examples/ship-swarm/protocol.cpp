...
void send_entity_input(socketwire::ReliableConnection* connection,
  ...
  const auto thrSteerPacked = static_cast<std::uint8_t>(
    (thrPacked.packedVal << 4) | steerPacked.packedVal);
...
void deserialize_entity_input(const void* data, std::size_t size,
  ...
  static const std::uint8_t NEUTRAL_PACKED_VALUE =
    pack_float<std::uint8_t>(0.f, -1.f, 1.f, 4);
  float4bitsQuantized thrPacked(thrSteerPacked >> 4);
  float4bitsQuantized steerPacked(thrSteerPacked & 0x0f);
  thr = thrPacked.packedVal == NEUTRAL_PACKED_VALUE
          ? 0.f
          : thrPacked.unpack(-1.f, 1.f);
  steer = steerPacked.packedVal == NEUTRAL_PACKED_VALUE
            ? 0.f
            : steerPacked.unpack(-1.f, 1.f);
}
