...
class ClientHandler final : public IReliableConnectionHandler {
 public:
  ...
  void OnReliableReceived(std::uint8_t, const void* data,
    ...
    if (!typeValue || static_cast<large_message_demo::MessageType>(
                        *typeValue) != large_message_demo::MessageType::BlobAck)
      return;
    ...
    if (!receivedSize || !expectedChecksum || !actualChecksum) return;

    ackOk = *receivedSize == large_message_demo::K_PAYLOAD_SIZE &&
            *expectedChecksum == *actualChecksum;
    std::printf("server ack: size=%u checksum=%08x status=%s\n", *receivedSize,
                *actualChecksum, ackOk ? "ok" : "bad");
  }
